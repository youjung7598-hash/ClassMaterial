...
class Car:
    def __init__(self, color): # 객체(인스턴스) 초기화
        self.color = color # 속성(특징) 지정

    def start(self): # 인스턴스 메서드(객체가 수행하는 동작)
...
| 단계  | 이름               | 의미            |
| --- | ---------------- | ------------- |
| 1   | `Car("빨간색")`     | 객체(인스턴스) 생성   |
| 2   | `__init__()`     | 생성된 객체를 초기화   |
| 3   | `my_car.start()` | 객체의 동작(로직) 실행 |

![[Pasted image 20260102171230.png]]
...
```
- `def` → 함수 정의
- `self` → 객체 자신(필수 첫 번째 인자)
- 들여쓰기 주의!

📖 문법, 구문3 (syntax): 객체가 생성될 때 자동 실행되는 메서드
```python
# 생성자 __init__()
class 클래스이름:
...
class Monster:
    def __init__(self, name):
        self.name = name    # name을 객체 속성에 저장
```
- `__init__` → 특별한 메서드(초기화 담당)
- `self.속성` → 객체 안에 저장되는 값
- 오른쪽 매개변수 → 전달받은 값
...
```python
# Monster라는 클래스를 정의 (몬스터 설계도)
class Monster:
    # 생성자: 객체가 만들어질 때 자동으로 실행됨
    # name이라는 값을 받아서 self.name에 저장
    def __init__(self, name):
        self.name = name   # 각 몬스터마다 가진 이름 속성

    # 메서드: 몬스터가 할 수 있는 동작(기능)
    def say(self):
        print(f"나는 {self.name}")   # 자신의 이름을 말함

# Monster 클래스를 이용해 실제 객체(인스턴스) 생성
shark = Monster("상어")    # 이름이 '상어'인 몬스터 생성
wolf = Monster("늑대")    # 이름이 '늑대'인 몬스터 생성

# shark 객체가 say() 메서드를 실행
shark.say()               # 출력: 나는 상어    
```

</> 의사코드
```python
1. Monster라는 설계도를 만든다.
   - 이 설계도에는
     (1) 몬스터가 태어날 때 이름을 저장하는 방법(__init__)
     (2) 몬스터가 자기 이름을 말하게 하는 방법(say)
     이 두 가지 기능이 들어 있다.

2. 몬스터가 태어날 때(__init__ 실행될 때) 하는 일:
   - 새 몬스터(self)를 하나 만든다.
   - 호출한 사람이 준 name 값(예: "상어")을 받는다.
   - 새 몬스터(self)에 name이라는 이름표로 붙여둔다.
     (self.name = name)

3. 실제 몬스터를 만든다.
   - shark = Monster("상어") 를 실행하면
     (1) Monster 설계도를 보고 새 몬스터(shark)를 하나 만든다.
     (2) __init__을 자동으로 호출해서 shark.name에 "상어"를 저장한다.

   - wolf = Monster("늑대") 도 같은 방식으로 만든다.
     (wolf.name에 "늑대" 저장)

4. 몬스터에게 말하라고 시킨다.
   - shark.say() 를 실행하면
     (1) shark가 가진 name("상어")을 꺼내서
     (2) "나는 상어" 를 화면에 출력한다.
```
...
student2 = Student("김철수", 80, 75, 85, 90)
student3 = Student("이영희", 70, 65, 75, 80)
...
# 이 객체가 어떤 클래스인지 파이썬이 문자열로 보여주는 형식
print(type(student1))
```
...
(1) 클래스와 인스턴스
- 클래스(`Student`)  
    → “학생”을 표현하기 위한 설계도 / 타입(type)  
    → 이 설계도로 실제 학생 객체를 찍어낸다.
    
- 인스턴스(객체)  
    → `student1`, `student2`, `student3` 처럼  
    → `Student` 설계도로 만든 실제 데이터 덩어리
(2) `__init__` 과 인스턴스 변수
```python
def __init__(self, name, korean, english, math, science):
    self.name = name
    self.korean = korean
    ...
    self.science = science
```
- 이 함수는 객체가 생성된 직후 자동으로 호출된다.
- `self.name`, `self.korean` … 은 인스턴스 변수(객체가 가진 속성)**.
- 오른쪽의 `name`, `korean` … 은 함수로 전달된 값(매개변수)**.
- `self.변수 = 매개변수`  
    → “이 객체(self)의 속성에, 전달받은 값을 저장하라”는 뜻.
    
📌 용어 정리:
- 속성 할당(Attribute Assignment)  
    → `self.name = name` 처럼 객체의 속성에 값을 넣는 행위
- 인스턴스 변수 초기화(Initialization)  
    → `__init__` 안에서 인스턴스 변수에 초기값을 넣어 준비 상태로 만드는 것
...
def get_sum(self):
    return self.korean + self.english + self.math + self.science
```
- `get_sum` → 그 학생의 **총점**을 계산하는 메서드

```python
def get_average(self):
    return self.get_sum() / 4
```
- `get_average` → `get_sum()`을 다시 호출해서 평균 계산
- 나중에 과목이 늘어도 `get_sum`만 고치면 되도록 재사용하는 구조

```python
def print_info(self):
    print(f"{self.name}\t총점: {self.get_sum()}\t평균: {self.get_average():.2f}")
```
- `print_info` → 이 학생 한 명의 정보 출력
- `self.get_sum()` / `self.get_average()`처럼  
    → 같은 객체 안의 다른 메서드를 호출하는 구조
- `:.2f` → 평균을 소수점 둘째 자리까지 출력
...
1. 새 `Student` 객체 생성
2. `__init__` 자동 호출
3. `self.name = "홍길동"` 등 값 저장
4. 그 객체를 `student1` 변수에 넣음

```python
student1.print_info()
```
- `student1`이 가진 `print_info` 기능을 실행해라라는 뜻
- 점(`.`) 앞: 어떤 객체인지
- 점(`.`) 뒤: 그 객체의 속성/메서드 이름
...
이 문자열의 의미는:
- `class` → 이건 클래스(타입)입니다.
- `__main__` → 현재 실행 중인 파일(모듈)의 이름  
    (지금 이 코드가 있는 파일을 직접 실행하면, 파이썬은 그 파일을 `__main__`이라고 부름)
- `Student` → 우리가 정의한 클래스 이름
👉 한 문장으로 말하면:
`type(student1)`을 출력하면, `student1`이 어떤 설계도(어떤 클래스)로 만들어졌는지 그 출신 
클래스를 보여주는 거예요.
...
`사용:`
```python
a1 = Article("공지사항", "오늘은 중요한 공지가 있습니다...")
print(a1.summary())
```
📌 특징
- 내가 직접 `__init__()`에서 값 저장
- `self.title`, `self.content` = 객체가 가진 데이터

---
같은 기능을 Django 방식으로 만든 클래스
`모델 클래스 정의 (models.py)`
```python
...
| 비교 대상      | 일반 클래스                | Django 모델                     |
| ---------- | --------------------- | ----------------------------- |
| 정체         | 단순 설계도                | DB와 연결되는 설계도                  |
| 저장 위치      | 메모리                   | 데이터베이스                        |
| `__init__` | 내가 직접 만듦              | Django가 자동 제공                 |
| 속성 정의      | `self.title = title`  | `title = models.CharField()`  |
| 데이터 생성     | `Article("제목", "내용")` | `Article.objects.create(...)` |
| 메서드 호출     | `a.summary()`         | `a.summary()` (동일)            |
| self 의미    | 객체 자신                 | 객체 자신 (동일)                    |

`(1)` `title = models.CharField()` — 왜 self가 없나요?
```python
title = models.CharField(max_length=100)
```

객체가 실제로 만들어질 때 Django가 자동으로
```python
self.title
self.content
self.created_at
...
`(2)` summary는 왜 `self`가 있을까?
```python
def summary(self):
    return self.content[:50]
```
왜냐하면
	이 Article 객체 자신의 내용(self.content)을 잘라서 반환하라는 뜻이기 때문입니다.
그래서
✔ 객체 기준 동작이므로 self 필요  
✔ title/content 같은 값에 접근해야 하므로 self 필요

`(3)` 템플릿에서 왜 괄호가 없나요? 호출방식이 다르기때문입니다.
```html
{{ article.summary }}
```
...
---
Django에서 리스트 화면 클래스 = 하나의 작업을 위한 설계도
...
1️⃣ DB에서 Article 데이터 가져오기  
2️⃣ 정렬/필터 같은 처리  
3️⃣ 가져온 데이터를 템플릿에 전달  
4️⃣ HTML로 렌더링해서 사용자에게 보여주기

👉 이 모든 과정을 하나의 클래스 안에서 처리합니다.

---
API도 똑같은 개념 (HTML 대신 JSON일 뿐)

API 클래스도:
1️⃣ DB에서 데이터 가져오고  
2️⃣ 직렬화(Serializer) 하고  
3️⃣ JSON 응답 반환

이 일을 전부 같은 클래스에서 처리합니다.
(ex. DRF `APIView`, `ViewSet`)
